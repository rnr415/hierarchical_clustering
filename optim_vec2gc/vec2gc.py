...
class HierarchicalClustering:
    ...
    def create_graph_from_similarity_matrix(self, similarity_matrix: np.ndarray) -> nk.Graph:
        """
        Create NetworKit graph directly from a similarity matrix.
        If element (i,j) > 0, an edge is created between nodes i and j with weight equal to the matrix value.
        
        Args:
            similarity_matrix: n x n matrix where element (i,j) represents the similarity/weight
                             between nodes i and j. If (i,j) = 0, no edge is created.
        ...
        # Get weights for all potential edges
        edge_weights = similarity_matrix[i_indices, j_indices]
        
        # Find edges with positive weights
        valid_edges = edge_weights > 0.0
        
        # Filter to get only valid edge indices and weights
        valid_i = i_indices[valid_edges]
        valid_j = j_indices[valid_edges]
        valid_weights = edge_weights[valid_edges]
        
        # Add edges to NetworKit graph
        if len(valid_i) > 0:
            for i, j, weight in zip(valid_i, valid_j, valid_weights):
                nk_graph.addEdge(int(i), int(j), float(weight))
    ...
    def detect_communities(self, nk_graph: nk.Graph) -> Tuple[List[List[int]], float]:
        """
        ...
        Returns:
            Tuple of (List of lists containing node IDs in communities, modularity score)
            Each inner list contains the node IDs belonging to that community
        """
        try:
            ...
            # Convert partition to communities as list of lists
            communities = []
            for community_id in range(partition.numberOfSubsets()):
                # Get all nodes in this community and convert to list
                community_nodes = list(partition.getMembers(community_id))
            ...
            return [list(range(nk_graph.numberOfNodes()))], 0.0
